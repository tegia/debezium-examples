...
import io.debezium.examples.aggregation.model.NewAddress;
...
public class StreamingAggregatesDDD {
    ...
    public static void main(String[] args) {
        ...
        final Serde<NewAddress> newAddressSerde = SerdeFactory.createDbzEventJsonPojoSerdeFor(NewAddress.class,false);
        ...
        // KTable<DefaultId, NewAddress> dddNewAddress = addressStream
        //         .groupByKey(Serialized.with(defaultIdSerde, addressSerde))
        //         .aggregate(
        //                 () -> new NewAddress(),
        //                 (DefaultId service_address_id, Address address, NewAddress newAddress) -> {
        //                         newAddress.update(address.getStreet()+ ','+ address.getCity(), service_address_id);
        //                         return newAddress;
        //                 },
        //                 Materialized.<DefaultId,NewAddress,KeyValueStore<Bytes, byte[]>>
        //                                 as(childrenTopic+"_table_temp")
        //                                     .withKeySerde(defaultIdSerde)
        //                                         .withValueSerde(newAddressSerde)
        //         );
        ...
        KTable<DefaultId,LatestAddress> tempTable = addressStream
                ...
                .aggregate(
                        ...
                        //     latest.update(address,addressId,new DefaultId(address.getCustomer_id()), address.getStreet()+ ','+ address.getCity());
                            latest.update(addressId, address.getStreet()+ ','+ address.getCity());
                            return latest;
                        },
                        Materialized.<DefaultId,LatestAddress,KeyValueStore<Bytes, byte[]>>
                                        as(childrenTopic+"_test_temp")
                                            .withKeySerde(defaultIdSerde)
        ...
        // dddNewAddress.toStream().to("final_ddd_aggregates",
        //                             Produced.with(defaultIdSerde,(Serde)newAddressSerde));

        // dddNewAddress.toStream().print(Printed.toSysOut());

        tempTable.toStream().to("final_ddd_aggregates",
                                    Produced.with(defaultIdSerde,(Serde)latestAddressSerde));
